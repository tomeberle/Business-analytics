...
tweets_df1['Text'] = tweets_df1['Text'].replace(r'[^a-z ]', '', regex=True).replace("'", '') # getting rid of everything except letters (a-z)
tweets_df1['Text'] = tweets_df1['Text'].apply(lambda x: ' '.join([y for y in x.split() if 'http' not in y])) # getting rid of the links
tweets_df1['Datetime'] = tweets_df1['Datetime'] + datetime.timedelta(hours = +2) # fixing time to Paris time
tweets_df1['Date'] = [d.date() for d in tweets_df1['Datetime']] # splitting datetime into date and time
...
# Export as CSV
tweets_df1.to_csv (r'C:\Users\patri\Documents\GitHub\Business-analytics\twitter_sentiment.csv', encoding='utf-8-sig')
